...
intensity_scan_pos_R = linspace(0,L/2,2000);
...
for j=1:length(detuning_range)
    for l=1:length(gain_range)
        % Matrices for right handed medium
        M_RHM1 = cwt_from_reduced_variables(p_R, 0, L/2, k_R(l,j), kappa_R, deltak_R(l,j));
        M_RHM2 = cwt_from_reduced_variables(p_R, psi, L/2, k_R(l,j), kappa_R, deltak_R(l,j));
        % Matrix for the two LH media
        M_LHM1 = cwt_from_reduced_variables(p_L, 0, L_L, k_L(l,j), kappa_L, deltak_L(l,j), true);
        M_LHM2 = cwt_from_reduced_variables(p_L, psi, L_L, k_L(l,j), kappa_L, deltak_L(l,j), true);
        ...
        M_interface_LR = interface_left_to_right_from_reduced_variables(p_L, 0, k_L(l,j), kappa_L, L_L, p_R, 0, k_R(l,j), kappa_R, 0, k_0(l,j));
        % Matrix for the interface RR
        M_interface_RR = interface_chiral_same_handedness_from_reduced_variables(p_R, kappa_R, n_bar_R(l,j), 0, p_R, kappa_R, n_bar_R(l,j), psi, L/2, k_0(l,j));
        ...
        M_interface_RL = interface_left_to_right_from_reduced_variables(p_L, 0, k_L(l,j), kappa_L, 0, p_R, psi, k_R(l,j), kappa_R, L/2, k_0(l,j))^-1;
        ...
        M_cavity = M_interface_2*M_LHM2*M_interface_RL*M_RHM2*M_interface_RR*M_RHM1*M_interface_LR*M_LHM1*M_interface_1;
        lasing_coeff(l,j) = abs(inv(det(M_cavity(3:4,3:4))));
        cavities(:,:,l,j) = M_cavity;
        cavities_inside_h(:,:,l,j) = M_RHM2*M_interface_RR*M_RHM1;
        ...
        M_oseen = T^-1*M_int_2^-1*M_L2*M_R2*M_R1*M_L1*M_int_1*T;
...
nb_scan_pos = 2*nb_scan_pos_L + 2*nb_scan_pos_R;
...
for u=1:length(row)
    ...
    M_interface_LR = interface_left_to_right_from_reduced_variables(p_L, 0, k_L(l,j), kappa_L, L_L, p_R, 0, k_R(l,j), kappa_R, 0, k_0(l,j));
    % Transfer matrix for interface Right to Right
    M_interface_RR = interface_chiral_same_handedness_from_reduced_variables(p_R, kappa_R, n_bar_R(l,j), 0, p_R, kappa_R, n_bar_R(l,j), psi, L/2, k_0(l,j));
    % Transfer matrix at interface right to left
    M_interface_RL = interface_left_to_right_from_reduced_variables(p_L, psi, k_L(l,j), kappa_L, 0, p_R, psi, k_R(l,j), kappa_R, L/2, k_0(l,j))^-1;
    ...
    % Scan the first RH medium
    for v=1:nb_scan_pos_R
        % length of the partial slab
        len = intensity_scan_pos_R(v);

        % Corresponding matrix
        M_RHM = cwt_from_reduced_variables(p_R, 0, len, k_R(l,j), kappa_R, deltak_R(l,j));
        % Field at z
        F = M_RHM*(M * F_0);
        % Intensity at z
    ...
    M = M_interface_RR * cwt_from_reduced_variables(p_R, 0, L/2, k_R(l,j), kappa_R, deltak_R(l,j)) * M;

    % Scan the second RH medium
    for v=1:nb_scan_pos_R
        % length of the partial slab
        len = intensity_scan_pos_R(v);
    ...
    % Shortcut to store the matrix corresponding to the first LH medium and the RH medium
    M = M_interface_LR * cwt_from_reduced_variables(p_R, psi, L/2, k_R(l,j), kappa_R, deltak_R(l,j)) * M;
    ...
    for v=1:nb_scan_pos_L
        ...
        % Corresponding matrix
        M_LHM = cwt_from_reduced_variables(p_L, psi, len, k_L(l,j), kappa_L, deltak_L(l,j), true);
        % Field at z
        F = M_LHM*(M * F_0);
        % Intensity at z
        intensities(v+nb_scan_pos_L+2*nb_scan_pos_R,res_start:res_stop) = abs(F).^2;
    end
end
...
for u=1:length(row_oseen)
    ...
    M_R1 = oseen(p_R, epsilon_a_R(l,j), epsilon_b_R(l,j), 0, L/2, k_0(l,j)) * M_L1;
    ...
    for v=1:nb_scan_pos_R
        ...
        len = intensity_scan_pos_R(v);
        % Corresponding matrix
        M_R = oseen(p_R, epsilon_a_R(l,j), epsilon_b_R(l,j), psi, len, k_0(l,j));
        % Intensity at z
        intensities_oseen(v+nb_scan_pos_L+nb_scan_pos_R,res_start:res_stop) = abs(M_R*M_R1*F_0).^2;
    end
    % Shorthand for LH+RH1+RH2 medium
    M_R2 = oseen(p_R, epsilon_a_R(l,j), epsilon_b_R(l,j), psi, L/2, k_0(l,j)) * M_R1;
    ...
    for v=1:nb_scan_pos_L
        ...
        len = intensity_scan_pos_L(v);
        % Corresponding matrix
        M_L = oseen(p_L, epsilon_a_L(l,j), epsilon_b_L(l,j), psi, len, k_0(l,j));
        % Intensity at z
        intensities_oseen(v+nb_scan_pos_L+2*nb_scan_pos_R,res_start:res_stop) = abs(M_L*M_R2*F_0).^2;
    end
end
...
savefig(gcf, 'hybrid_defect_cavity', 'lasing', 'png')
...
savefig(gcf, 'hybrid_defect_cavity', 'surface', 'eps', 'epsc')
...
savefig(gcf, 'hybrid_defect_cavity', 'contour', 'eps', 'epsc')
...
savefig(gcf, 'hybrid_defect_cavity', 'lasing_oseen', 'png')
...
ylabel('Gain','interpreter','latex')
zlabel('$-\log(\mathrm{det})$','interpreter','latex')
savefig(gcf, 'hybrid_defect_cavity', 'surface_oseen', 'eps', 'epsc')
...
savefig(gcf, 'hybrid_defect_cavity', 'contour_oseen', 'eps', 'epsc')
...
X_intensities = cat(2, intensity_scan_pos_L, intensity_scan_pos_R+L_L, intensity_scan_pos_R+L_L+L/2, intensity_scan_pos_L+L+L_L) ./ (2*L_L+L);
...
for u=1:length(row)
    ...
    h_fr1 = fill([L_L/(L+2*L_L) 0.5 0.5 L_L/(L+2*L_L)], [max(ylim) max(ylim) min(ylim) min(ylim)],'c','FaceAlpha',0.1, 'LineStyle', 'none');
    h_fr2 = fill([0.5 (L+L_L)/(L+2*L_L) (L+L_L)/(L+2*L_L) 0.5], [max(ylim) max(ylim) min(ylim) min(ylim)],'b','FaceAlpha',0.1, 'LineStyle', 'none');
...
lgd = legend(hL,[h1;h2;h3;h4;h5;h_fl;h_fr1;h_fr2],'I','I_L^+','I_R^+','I_L^-','I_R^-','LH medium', 'RH medium 1', 'RH medium 2','NumColumns',3);
set(lgd,'position',poshL);
axis(hL,'off');

savefig(gcf, 'hybrid_defect_cavity', 'intensity_distribution', 'png')
...
ylabel('Gain', 'interpreter', 'latex')
daspect([1 1 1])
savefig(gcf, 'hybrid_defect_cavity', 'modes_found', 'eps', 'epsc')
...
poshL = get(hL,'position');
lgd = legend(hL,[h1;h2;h3;h4;h5;h_fl;h_fr1;h_fr2],'$I$','$I_L^+$','$I_R^+$','$I_L^-$','$I_R^-$','LH medium', 'RH medium 1', 'RH medium 2','NumColumns',3, 'interpreter', 'latex');
set(lgd,'position',poshL);
axis(hL,'off');

savefig(gcf, 'hybrid_defect_cavity', 'intensity_distribution', 'eps', 'epsc')
...
hL = subplot(n_rows+2,nb_col_mode,[u+1+2*nb_col_mode nb_col_mode*(n_rows+2)]);
poshL = get(hL,'position');
lgd = legend(hL,[h1;h_fl;h_fr1;h_fr2],'I','LH medium', 'RH medium 1', 'RH medium 2');
set(lgd,'position',poshL);
axis(hL,'off');

savefig(gcf, 'hybrid_defect_cavity', 'intensity_distribution_oseen', 'png')
...
ylabel('Gain', 'interpreter', 'latex')
daspect([1 1 1])
savefig(gcf, 'hybrid_defect_cavity', 'modes_found_oseen', 'eps', 'epsc')
...
hL = subplot(n_rows,nb_col_mode,[u+1, nb_col_mode*n_rows]);
poshL = get(hL,'position');
lgd = legend(hL,[h1;h_fl;h_fr1;h_fr2],'I','LH medium', 'RH medium 1',  'RH medium 2', 'interpreter', 'latex');
set(lgd,'position',poshL);
axis(hL,'off');

savefig(gcf, 'hybrid_defect_cavity', 'intensity_distribution_oseen', 'eps', 'epsc')
